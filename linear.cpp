...
static int solve_l2r_l1l2_svc_kernel(const problem *prob, const parameter *param, double *w, double *alpha, double Cp, double Cn, int max_iter=DEFAULT_MAX_ITER, const kernel_parameter *kernel_param=0)
{
	...
	int i, s, iter = 0, iter_minibatch = 0;
	double C, d, G;
	double *QD = new double[l];
	int *index = new int[l];
	schar *y = new schar[l];
	...
	// Prepare minibatches (if kernelized)
	SVC_Q_NoCache *svc_q = NULL;
	int num_minibatches = 1;
	int num_instances_per_mb = l;
	int num_instances_last_mb = l;
	if(kernel_param){
		svc_q = new SVC_Q_NoCache(*prob,*kernel_param,y);
		num_instances_per_mb = (int)((kernel_param->cache_size * 1048576.0) / sizeof(Qfloat) / l);
		if(num_instances_per_mb > l) num_instances_per_mb = l;
		if(num_instances_per_mb <= 0) num_instances_per_mb = 1;
		num_minibatches = (l + num_instances_per_mb - 1) / num_instances_per_mb; // "l/num_instances_per_mb" with rounded up
		if(num_minibatches * num_instances_per_mb == l){
			num_instances_last_mb = num_instances_per_mb;
		}else{
			num_instances_last_mb = (num_minibatches * num_instances_per_mb) - l;
		}
	}
	...
	// Variables to store information of minibatch
	int mbid, mb_start, mb_size;
	int initialized_for_non_minibatch = 0;
	// If minibatches are not employed, recomputation of the kernel matrix is not needed.
	// However, in this case, different from the case with minibatches, the indices of
	// the matrix Q must be stored in the order of original dataset, not the order of the
	// minibatch instances.

	Qfloat ** Q_sub = 0;
	if(kernel_param){
		Q_sub = Malloc(Qfloat *, num_instances_per_mb);
		for(s = 0; s < num_instances_per_mb; ++s){
			Q_sub[s] = Malloc(Qfloat, l);
		}
	}
	...
	while (iter < max_iter)
	{
		...
		int alpha_updates = 0;
		for(mbid = 0; mbid < num_minibatches; ++mbid){
			mb_start = num_instances_per_mb * mbid;
			mb_size = ((mbid == num_minibatches - 1) ? num_instances_last_mb : num_instances_per_mb);
			active_size = mb_size;
			fprintf(stderr, "\nIter = %d, Minibatch = %d/%d (#MinibatchInstances %d)\n", iter+1, mbid+1, num_minibatches, mb_size);

			// Create the kernel submatrix
			if(kernel_param){
				if(num_minibatches == 1){
					if(!initialized_for_non_minibatch){
						for(s = 0; s < mb_size; ++s){
							svc_q->get_Q(s, l, Q_sub[s]);
						}
						initialized_for_non_minibatch = 1;
					}
				}else{
					for(s = 0; s < mb_size; ++s){
						svc_q->get_Q(index[mb_start + s], l, Q_sub[s]);
					}
				}
			}

			while (iter_minibatch < max_iter)
			{
				PGmax_new = -INF;
				PGmin_new = INF;
				...
				for (s=0; s<active_size; s++)
				{
					...
					// From KKT condition, yi * (xi . w) = Qi . alpha
					...
					if(!kernel_param){
						...
						if(initialized_for_non_minibatch){
							for(int ii = 0; ii < l; ++ii){
								G += Q_sub[i][ii] * alpha[ii];
							}
						}else{
							for(int ii = 0; ii < l; ++ii){
								G += Q_sub[s][ii] * alpha[ii];
							}
						}
					}
					...
					if (alpha[i] == 0)
					{
						...
						{
							active_size--;
							swap(index[s+mb_start], index[active_size+mb_start]);
							if(kernel_param && !initialized_for_non_minibatch){
								swap(Q_sub[s], Q_sub[active_size]);
							}
							s--;
							continue;
						}
					...
					if(fabs(PG) > 1.0e-12)
					{
						...
						if(fabs(d) > 1.0e-12) ++alpha_updates;
						if(!kernel_param){
				...
				iter_minibatch++;
				if(iter_minibatch % 100 == 0)
					info(".");

				if(PGmax_new - PGmin_new <= eps &&
					fabs(PGmax_new) <= eps && fabs(PGmin_new) <= eps)
				{
					if(active_size == mb_size){
						break;
					}
					else
					{
		...
		iter++;
		fprintf(stderr, "Iter = %d, #updatedAlphas = %d\n", iter, alpha_updates);
		if(alpha_updates == 0) break;
	}
	...
	if(!kernel_param){
		...
		// alpha' Q alpha
		for(i=0; i<l; i++){
			svc_q->get_Q(i, l, Q_sub[0]);
			for(int ii=0; ii<l; ii++){
				v += alpha[i] * alpha[ii] * Q_sub[0][ii];
			}
		}
	}
...
static int solve_l2r_lr_dual_kernel_main(const problem *prob, const parameter *param, double *w, double *alpha, double Cp, double Cn, int max_iter=DEFAULT_MAX_ITER, const kernel_parameter *kernel_param=0)
{
	int l = prob->l;
	int w_size = prob->n;
	double eps = param->eps;
			...
			while (iter_minibatch < max_iter)
			{
				...
				for (s=0; s<l; s++)
				{
					...
					if(!kernel_param){
						...
						if(initialized_for_non_minibatch){
							for(int ii = 0; ii < l; ++ii){
								ywTx += Q_sub[i][ii] * alpha[2*ii];
							}
						}else{
							for(int ii = 0; ii < l; ++ii){
								ywTx += Q_sub[s][ii] * alpha[2*ii];
							}
						}
					}
	...
	if(!kernel_param){
		...
		for(i=0; i<l; i++){
			svc_q->get_Q(i, l, Q_sub[0]);
			for(int ii=0; ii<l; ii++){
				v += alpha[2*i] * alpha[2*ii] * Q_sub[0][ii];
			}
		}
	}
...
{
	double *alpha = new double[2 * prob->l];
	int iter = solve_l2r_lr_dual_kernel_main(prob, param, w, alpha, Cp, Cn, max_iter);
	delete [] alpha;
	return iter;
}

static int solve_l2r_lr_dual_kernel(const problem *prob, const parameter *param, double *w, double *alpha, double Cp, double Cn, int max_iter, const kernel_parameter *kernel_param)
{
	double *alpha_tmp = new double[2 * prob->l];
	int iter = solve_l2r_lr_dual_kernel_main(prob, param, w, alpha_tmp, Cp, Cn, max_iter, kernel_param);
	for(int i = 0; i < prob->l; ++i){
		alpha[i] = alpha_tmp[2*i];
	}
	delete [] alpha_tmp;
	return iter;
}
...
static void train_one(const problem *prob, const parameter *param, const kernel_parameter *kernel_param, double *w, double Cp, double Cn)
{
	...
	switch(solver_type)
	{
		...
		{
			if(kernel_param){
				iter = solve_l2r_lr_dual_kernel(prob, param, 0, w, Cp, Cn, dual_solver_max_iter, kernel_param);
			}else{
